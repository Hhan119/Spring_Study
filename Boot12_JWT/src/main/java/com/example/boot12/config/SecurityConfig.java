...
//	}
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity, 
			AuthSuccessHandler successHandler, AuthFailHandler failHandler, // 객체 주입을 받아도 되고, 매개변수로 선언해도 값이 들어온다.
			CookieRequestCache cookCache) throws Exception{
		//화이트 리스트를 미리 배열에 넣어두기
		String[] whiteList= {"/", "/user/signup_form", "/user/signup", "/editor/images/*", 
				"/user/loginform", "/user/login_fail", "/user/expired","/test/login", "/api/test/login", "/api/auth"};
		...
		.addFilterBefore(jwtFilter, UsernamePasswordAuthenticationFilter.class) // 모든 요청에 대해서 토큰 관련 필터링을 함 
