...
public class GalleryController {
	...
	@ResponseBody
	@GetMapping(value="/gallery/images/{imageName}", produces = {MediaType.IMAGE_GIF_VALUE, MediaType.IMAGE_JPEG_VALUE, MediaType.IMAGE_PNG_VALUE})
	//@GetMapping("/gallery/images/{imageName}")
	public byte[] image(@PathVariable("imageName") String name) throws IOException {
		// imageNmae 경로 변수에 들어 있는 문자열이 매개변수 String name에 전달된다.
		// 읽어들일 파일의 절대 경로
	...
	/*
	 *	<input type="file" name="image">의 name 속성의 value가 image이기 때문에,
	 *	MultipartFile image로 매개 변수를 선언하면 된다.  
	 */
